...
export default {
  ...
  methods: {
    ...
    async getParamsData () {
      ...
      // 根据所选的分类id，和当前所选的面板，获取对应的参数
      const response = await this.$http.get(`/v1/categories/${this.cateId}/attributes`, { params: { sel: this.activeName } })
    ...
    async addParams () {
      this.$refs.addFormRef.validate(async (valid) => {
        ...
        const response = await this.$http.post(`/v1/categories/${this.cateId}/attributes`, {
    ...
    async showEditDialog (attrId) {
      ...
      // 查询当前参数的信息
      const response = await this.$http.get(`/v1/categories/${this.cateId}/attributes/${attrId}`, { params: { attr_sel: this.activeName } })
      if (response.data.meta.status !== 200) {
    ...
    editParams () {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        const response = await this.$http.put(`/v1/categories/${this.cateId}/attributes/${this.editForm.attr_id}`, {
    ...
    async removeParams (attrId) {
      ...
      const response = await this.$http.delete(`/v1/categories/${this.cateId}/attributes/${attrId}`)
      if (response.data.meta.status !== 200) {
    ...
    async saveAttrValues (row) {
      // 保存到后端
      const response = await this.$http.put(`/v1/categories/${this.cateId}/attributes/${row.attr_id}`, {
